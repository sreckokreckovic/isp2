...
iptables -A FORWARD -s $client_subnet -o $INET_IFACE -p tcp -m multiport --dports 22,80,443 -m state --state NEW -j ACCEPT
iptables -A FORWARD -s $client_subnet -d $server_subnet -p tcp -m multiport --dports 22,80,443 -m state --state NEW -j ACCEPT 
